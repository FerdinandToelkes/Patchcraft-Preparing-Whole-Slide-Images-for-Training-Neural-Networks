...
# sample train data
# screen -dmS ftoelkes sh -c 'docker run -it -u `id -u $USER` --rm -v /home/ftoelkes/code/patchcraft:/patchcraft -v /data/sqlite:/data ftoelkes_patchcraft ../generate.sh train_200um_patches --input_path=/data --mode=train --number_of_slides=10 --start_slide=0 --number_of_patches_per_slide=all --patch_size_um=200 --overlap=0.5 --number_of_repeated_patches=4 --perturbation_range=0.1 --no-highest_zoom_level; exec bash'

# sample test data
# screen -dmS sample_patches_100um sh -c 'docker run --name ftoelkes_run1 -it -u `id -u $USER` --rm -v /home/ftoelkes/code/patchcraft:/patchcraft -v /data/sqlite:/data fto_patchcraft ../generate.sh test_1024um_patches --input_path=/data --mode=test --number_of_slides=1 --start_slide=0 --number_of_patches_per_slide=all --patch_size_um=1024 --overlap=0.0 --number_of_repeated_patches=1 --perturbation_range=0.0 --no-highest_zoom_level; exec bash'
